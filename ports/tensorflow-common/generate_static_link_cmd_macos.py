...
with open(sys.argv[1], "r") as f_in:
    with open("static_link.sh", "w") as f_out:
        if os.path.isfile(f"{params_path}/libtensorflow_framework.{version}.dylib-2.params"):
            ...
            p_linker = re.compile(
                fr"^\s*.+cc_wrapper.sh.+(@bazel-out\S+libtensorflow{lib_suffix}\.\d+\.\d+\.\d+\.dylib-2\.params).*")
            f_out.write(
            ...
            p_cd = re.compile(r"^\((cd .*) && \\$")
            p_linker1 = re.compile(
                fr"^.*cc_wrapper.sh.+-shared.+-o (bazel-out\S+libtensorflow{lib_suffix}\.\d+\.\d+\.\d+\.dylib)")
            p_linker2 = re.compile(
                "^.*cc_wrapper.sh.+-shared.+-o (bazel-out\\S+libtensorflow_framework\\.\\d+\\.\\d+\\.\\d+\\.dylib)")
            f_out.write(
                "#!/bin/bash\n# note: ar/binutils version 2.27 required to support output files > 4GB\n")
            env = []
            parts = None
            for line in f_in:
                if line.startswith("(cd"):
                    ...
                    if m1:
                        tokens = line.split()
                        if parts is None:
                            parts = [t[16:] for t in tokens if t.startswith(
                                "-Wl,-force_load,")]
                        else:
                            m = p_cd.match(env[0])
                            f_out.write(m.group(1) + "\n")
                            tmp = [t[16:] for t in tokens if t.startswith(
                                "-Wl,-force_load,")]
                            old = set(parts)
